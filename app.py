...
with tab_month:
    ...
    if not raw_hilo.empty:
        ...
        # Highlight Today's Row while is_today still exists
        def highlight_today(row):
            if row["is_today"]:
                return ["background-color: rgba(0, 119, 182, 0.25);"] * len(row)
            return [""] * len(row)

        # Apply styling to full month_df, then hide 'is_today' column using Styler.hide()
        styled_df = (
            month_df.style.apply(highlight_today, axis=1)
            .hide(subset=["is_today"], axis="columns")
        )
