    ...
    if not raw_hilo.empty:
        ...
        while current_day <= end_date:
            ...
            def format_tide(tide_row):
                ...
                # Bold time using markdown bold (**), non-bold height
        ...
        # Highlight Today's Row directly using Styler on month_df
        def highlight_today(row):
            if row["is_today"]:
                return ["background-color: #e0f2fe; font-weight: bold;"] * len(
                    row
                )
            return [""] * len(row)

        styled_df = month_df.style.apply(highlight_today, axis=1)

        # Display via Streamlit native dataframe with column formatting enabled
        st.dataframe(
            styled_df,
            column_config={
                "is_today": None  # Hides internal flag column cleanly
            },
