...
# --- Interactive Map Construction (Compatible with all Plotly versions) ---
map_lats, map_lons, map_names, map_colors, map_sizes = [], [], [], [], []

for name, info in STATIONS.items():
    map_names.append(name)
    map_lats.append(info["lat"])
    map_lons.append(info["lon"])
    if name == st.session_state.selected_station:
        map_colors.append("#d90429")  # Highlight red
        map_sizes.append(16)
    else:
        map_colors.append("#0077b6")  # Default blue
        map_sizes.append(10)

fig_map = go.Figure()

# Version-agnostic Map Trace
if hasattr(px, "scatter_map"):
    fig_map = px.scatter_map(
        lat=map_lats,
        lon=map_lons,
        hover_name=map_names,
        zoom=5.5,
    ...
    fig_map.add_trace(
        go.Scattermapbox(
            ...
            lon=map_lons,
            mode="markers",
            marker=dict(size=map_sizes, color=map_colors),
            text=map_names,
            hoverinfo="text",
        )
    )
...
if selected_point and "selection" in selected_point:
    ...
    if points:
        ...
        clicked_station = map_names[clicked_index]
